...
Official OpenBioFigure artifacts are built only by the repository's `Release` GitHub Actions workflow after a version tag is pushed. A clean GitHub-hosted `windows-latest` runner checks out the exact tagged commit, installs locked pnpm and Cargo dependencies, builds the release-mode application, and launches that real Tauri binary through WebdriverIO before the official Tauri GitHub Action may package it. The release is not created unless source policy, privacy, asset validation, unit/integration tests, browser tests, and the desktop smoke test all pass.
...
The Windows job launches `src-tauri/target/release/openbiofigure.exe` with the external Tauri WebDriver on the hosted runner. The smoke test must render Home, expose **New figure**, enter the editor, load a bundled scientific asset, and show its provenance. Failure artifacts include the WebdriverIO logs and a screenshot. Packaging and publication are downstream of this gate.
