...
The current catalog contains 733 SVGs from Bioicons: 409 CC0-1.0 assets, 323 CC-BY-4.0 assets, and one individually reviewed CC-BY-3.0 asset. The snapshot is reproducibly derived from the signed upstream commit recorded in `providers/bioicons.json`; it accepts only files with matching per-icon metadata, an identified creator, an allowlisted license directory, and complete provenance. Exact upstream paths, creators, licenses, source URLs, retrieval dates, revisions, and sanitized-file SHA-256 hashes are recorded in `catalog.json`.
